...
function handleCellInteraction(cell, x, y) {
    if (cell.soul) {
        ...
        gameState.player.exp += gained;
        gameState.player.lostExp = Math.max(0, gameState.player.lostExp - gained);
        gameState.world.soulRemnants = gameState.world.soulRemnants.filter(s => !(s.x === x && s.y === y));
        delete cell.soul;
